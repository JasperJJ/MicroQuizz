package com.jasper.microquizz.nfc.record;
...
public class SmartPoster implements ParsedNdefRecord {
	 ...
	 * NFC Forum Smart Poster Record Type Definition section 3.2.1.
	 *
	 * "The URI record. This is the core of the Smart Poster, and all other records are just
	 * metadata about this record. There MUST be one URI record and there MUST NOT be more than
	 * one."
	 */
	 ...
	 * NFC Forum Smart Poster Record Type Definition section 3.2.1.
	 *
	 * "The Action record. This record describes how the service should be treated. For example, the
	 * action may indicate that the device should save the URI as a bookmark or open a browser. The
	 * Action record is optional. If it does not exist, the device may decide what to do with the
	 * service. If the action record exists, it should be treated as a strong suggestion; the UI
	 * designer may ignore it, but doing so will induce a different user experience from device to
	 * device."
	 */
	 ...
	 * NFC Forum Smart Poster Record Type Definition section 3.2.1.
	 *
	 * "The Type record. If the URI references an external entity (e.g., via a URL), the Type record
	 * may be used to declare the MIME type of the entity. This can be used to tell the mobile
	 * device what kind of an object it can expect before it opens the connection. The Type record
	 * is optional."
	 */
	...
	/**
	 * Returns the first element of {@code elements} which is an instance of {@code type}, or {@code
